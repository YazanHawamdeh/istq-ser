...
import { listRequests } from '../../store/actions';
...
export default function index() {
  ...
  const { list, total } = useSelector((state) => state.ORDERS_STORE);
  ...
  useEffect(() => {
    dispatch(listRequests({ sort, page: 1, docs }));
  }, [sort]);
  ...
  const handleFilters = (e) => {
    setMyFilters(e);
    dispatch(listRequests({ sort, page: 1, docs, filters: e }));
  };
