...
import { createRequest, toggleLoader, showSuccessAlert, showErrorAlert } from '../../../store/actions';
...
export default function Create({ show, close }) {
  ...
  // const { UNITS, RACK } = useSelector((state) => state.LOOKUPS_STORE);
  const { ILO_SWITCH, NETWORK_SWITCH, RACK, UNITS, ILO_SWITCH_PORT, NETWORK_SWITCH_PORT, RAM } = useSelector((state) => state.LOOKUPS_STORE);

  // useEffect(() => {
  //   let today = new Date();
  //   setState({ ...state, orderDate: moment(today).format('YYYY-MM-DD'), estimationDate: moment(today).format('YYYY-MM-DDThh:mm') });
  // }, []);
  ...
  const Submit = async () => {
    ...
    if (valid) {
      ...
      let { isSuccess, data, error } = await dispatch(createRequest(dataToSend));
      dispatch(toggleLoader());
      if (isSuccess) {
        dispatch(showSuccessAlert('New request has created'));
        setSubmited(false);
        setState({});
        setErrors({});
        hide();
  ...
  return (
    <Modal isOpen={show} toggle={hide} size="xl">
      <div className="modal-header">
        <h5 className="modal-title mt-0">Create Order</h5>
      ...
      <div className="modal-body">
        ...
        <Row>
          {/* <Col>
            <DateInput label="Order date" name="orderDate" value={state.orderDate} onChange={HandleChange} error={errors.orderDate} />
          </Col> */}
          {/* <Col>
            <DateTimeInput
        ...
        <Row>
          <Col>
            <SelectMulti label="Network Switch" options={NETWORK_SWITCH} onChange={(e) => handleMumtiple(e, 'netWrokSwitch')} />
            {errors.netWrokSwitch ? <small style={{ color: '#f57878' }}> {errors.netWrokSwitch} </small> : null}
          </Col>
          <Col>
            <SelectMulti label="Ilo Switch Port" options={ILO_SWITCH_PORT} onChange={(e) => handleMumtiple(e, 'iloSwitchPort')} />
            {errors.iloSwitchPort ? <small style={{ color: '#f57878' }}> {errors.iloSwitchPort} </small> : null}
          </Col>
          <Col>
            <SelectMulti label="Network Switch Port" options={NETWORK_SWITCH_PORT} onChange={(e) => handleMumtiple(e, 'networkSwitchPort')} />
            {errors.iloSwitchPort ? <small style={{ color: '#f57878' }}> {errors.iloSwitchPort} </small> : null}
          </Col>
        </Row>
        ...
        <Row>
          <Col>
            <Select label="Ram" name="ram" value={state.ram} onChange={HandleChange} options={RAM} />
          </Col>
        </Row>
      </div>
